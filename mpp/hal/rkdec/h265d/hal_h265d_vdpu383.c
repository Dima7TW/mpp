...
static MPP_RET hal_h265d_vdpu383_init(void *hal, MppHalCfg *cfg)
{
    ...
    reg_ctx->pps_buf_sz = 22 * sizeof(RK_U64); // 22 * 64 bits
...
static RK_S32 hal_h265d_vdpu383_rps(HalH265dCtx *reg_ctx, void *dxva, void *rps_buf)
{
    BitputCtx_t bp;
    RK_S32 fifo_len = 400;
    RK_S32 i = 0, j = 0;
    ...
    if (!dxva_ctx->pp.rps_update_flag) {
        if (reg_ctx->fast_mode)
            memcpy(rps_buf, reg_ctx->sw_rps_buf, fifo_len * sizeof(RK_U64));
        return 0;
    }

    mpp_set_bitput_ctx(&bp, (RK_U64*)reg_ctx->sw_rps_buf, fifo_len);
    ...
    hal_dbg_dump_data(reg_ctx->dbg_ctx, "rps_128bit.dat", (void *)reg_ctx->sw_rps_buf,
                      400 * 8, 128, 0, "w+");
...
static MPP_RET vdpu383_h265d_rcb_calc(void *context, RK_U32 *total_size)
{
...
static MPP_RET hal_h265d_vdpu383_gen_regs(void *hal,  HalTaskInfo *syn)
{
    ...
    if (vdpu383_setup_cur_stride_info(mframe, hw_regs, dxva_ctx->pp.chroma_format_idc)) {
    ...
    if (hw_regs->comm_addrs.reg168_decout_base == 0) {
        return 0;
    }
    ...
    hal_h265d_vdpu383_rps(reg_ctx, syn->dec.syntax.data, rps_ptr);
    ...
    vdpu383_init_ctrl_regs(hw_regs, MPP_VIDEO_CodingHEVC);
    ...
    for (i = 0; i < (RK_S32)MPP_ARRAY_ELEMS(dxva_ctx->pp.RefPicList); i++) {
        if (dxva_ctx->pp.RefPicList[i].bPicEntry != 0xff &&
            ...
            mpp_buf_slot_get_prop(cfg->frame_slots,
                                  ...
                                  SLOT_BUFFER, &framebuf);
            mpp_buf_slot_get_prop(cfg->frame_slots, dxva_ctx->pp.RefPicList[i].Index7Bits,
                                  SLOT_FRAME_PTR, &mframe);
            if (mpp_frame_get_thumbnail_en(mframe) == MPP_FRAME_THUMBNAIL_ONLY) {
                origin_buf = hal_bufs_get_buf(reg_ctx->origin_bufs,
                                              dxva_ctx->pp.RefPicList[i].Index7Bits);
    ...
    if ((reg_ctx->error_index[syn->dec.reg_index] == dxva_ctx->pp.CurrPic.Index7Bits) &&
        !dxva_ctx->pp.IntraPicFlag && !cfg->cfg->base.disable_error) {
    ...
    hw_regs->comm_paras.reg67_global_len = 0xc; //22 * 8;
    mpp_dev_set_reg_offset(cfg->dev, 131, reg_ctx->spspps_offset);

    /* rps */
    hw_regs->comm_addrs.reg129_rps_base = reg_ctx->bufs_fd;
    mpp_dev_set_reg_offset(cfg->dev, 129, reg_ctx->rps_offset);
    ...
    vdpu38x_h265d_rcb_setup(hal, dxva_ctx, syn, width, height,
                            &hw_regs->comm_addrs.rcb_regs, vdpu383_h265d_rcb_calc);
...

    {
        ...
        case MPP_FRAME_THUMBNAIL_ONLY:
            ...
            vdpu383_setup_down_scale(mframe, cfg->dev, &hw_regs->ctrl_regs, (void*)&hw_regs->comm_paras);
            break;
...
static MPP_RET hal_h265d_vdpu383_start(void *hal, HalTaskInfo *task)
{
    ...
    if (reg_ctx->fast_mode) {
        ...
        hw_regs = ( Vdpu383RegSet *)reg_ctx->g_buf[index].hw_regs;
    } else {
    ...
    do {
        ...
        vdpu383_dump_sw_regs(hw_regs, reg_ctx->dbg_ctx);
...
static MPP_RET hal_h265d_vdpu383_wait(void *hal, HalTaskInfo *task)
{
    ...
    if (hal_dbg_flag_en(reg_ctx->dbg_ctx, HAL_DBG_GET_REG))
        vdpu383_dump_hw_regs(hw_regs, reg_ctx->dbg_ctx);
...
MPP_DEC_HAL_API_REGISTER(hal_h265d_vdpu383)
