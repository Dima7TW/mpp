...
static MPP_RET hal_h265d_vdpu34x_init(void *hal, MppHalCfg *cfg)
{
    ...
    reg_ctx->fast_mode = cfg->cfg->base.fast_parse && cfg->support_fast_mode;

    hw_id = mpp_get_client_hw_id(VPU_CLIENT_RKVDEC);
    reg_ctx->is_v34x = (hw_id == HWID_VDPU34X || hw_id == HWID_VDPU38X);

    mpp_slots_set_prop(cfg->frame_slots, SLOTS_HOR_ALIGN, mpp_align_256_odd);
...

    {
        ...
        //!< malloc buffers
        ret = mpp_buffer_get(cfg->buf_group, &reg_ctx->bufs, ALL_BUFFER_SIZE(max_cnt));
        if (ret) {
        ...
        for (i = 0; i < max_cnt; i++) {
            reg_ctx->g_buf[i].hw_regs = mpp_calloc_size(void, sizeof(Vdpu34xRegSet));
...
{
    HalH265dCtx *reg_ctx = (HalH265dCtx *)hal;
    RK_U32 loop = (reg_ctx->fast_mode != 0) ? MPP_ARRAY_ELEMS(reg_ctx->g_buf) : 1;
    RK_U32 i;
    ...
    loop = (reg_ctx->fast_mode != 0) ? MPP_ARRAY_ELEMS(reg_ctx->rcb_buf) : 1;
    for (i = 0; i < loop; i++) {
...
static RK_S32 hal_h265d_v345_output_pps_packet(void *hal, void *dxva)
{
    ...
    RK_S32 width, height;
    HalH265dCtx *reg_ctx = ( HalH265dCtx *)hal;
    Vdpu34xRegSet *hw_reg = (Vdpu34xRegSet*)(reg_ctx->hw_regs);
    ...
    if (dxva_cxt->pp.scaling_list_enabled_flag) {
        ...
        hw_reg->comm_addr.reg180_scanlist_base = reg_ctx->bufs_fd;
        hw_reg->comm_gen.reg012.scanlist_addr_valid_en = 1;

        /* need to config addr */
        mpp_dev_set_reg_offset(reg_ctx->cfg->dev, 180, addr + reg_ctx->sclst_offset);
    }
...
static RK_S32 hal_h265d_output_pps_packet(void *hal, void *dxva)
{
    ...
    if (dxva_cxt->pp.ps_update_flag || dxva_cxt->pp.scaling_list_enabled_flag) {
        ...
        {
            RK_U8 *ptr_scaling = (RK_U8 *)mpp_buffer_get_ptr(reg_ctx->scaling_list_data);
            if (dxva_cxt->pp.scaling_list_data_present_flag) {
                addr = (dxva_cxt->pp.pps_id + 16) * 1360;
            } else if (dxva_cxt->pp.scaling_list_enabled_flag) {
                addr = dxva_cxt->pp.sps_id * 1360;
            ...
            RK_U32 fd = mpp_buffer_get_fd(reg_ctx->scaling_list_data);
            /* need to config addr */
            addr = fd | (addr << 10);
        ...
        for (i = 0; i < 64; i++)
            memcpy(pps_ptr + i * 80, reg_ctx->pps_buf, 80);
    } else if (reg_ctx->fast_mode) {
        for (i = 0; i < 64; i++)
            memcpy(pps_ptr + i * 80, reg_ctx->pps_buf, 80);
    }
                                  ...
                                  RK_S32 width, RK_S32 height, void *dxva)
{
    ...
    /* RCB_INTER_COL */
    rcb_bits = (tile_col_cut_num != 0) ? height * 22 : 0;
    ...
    /* RCB_FBC_ROW */
    if (hw_regs->comm_gen.reg012.fbc_e) {
    ...
    if (tile_col_cut_num) {
        if (hw_regs->comm_gen.reg012.fbc_e) {
            ...
            rcb_bits = height * (a * bit_depth + b + (bit_depth == 10 ? 16 : 0));
        }
    } else
        rcb_bits = 0;
...
static MPP_RET hal_h265d_vdpu34x_gen_regs(void *hal,  HalTaskInfo *syn)
{
    ...
    /* output pps */
    hw_regs = (Vdpu34xRegSet*)reg_ctx->hw_regs;
    memset(hw_regs, 0, sizeof(Vdpu34xRegSet));
    ...
    width = (dxva_cxt->pp.PicWidthInMinCbsY << log2_min_cb_size);
    height = (dxva_cxt->pp.PicHeightInMinCbsY << log2_min_cb_size);
    mv_size = vdpu34x_get_colmv_size(width, height, ctu_size, 16, 16, COLMV_COMPRESS_EN);
    if (reg_ctx->cmv_bufs == NULL || reg_ctx->mv_size < mv_size) {
        ...
        reg_ctx->mv_count = mpp_buf_slot_get_count(cfg->frame_slots);
...

    {
        ...
        mpp_buf_slot_get_prop(cfg->frame_slots, dxva_cxt->pp.CurrPic.Index7Bits,
                              SLOT_FRAME_PTR, &mframe);
        stride_y = mpp_frame_get_hor_stride(mframe);
        ver_virstride = mpp_frame_get_ver_stride(mframe);
        stride_uv = stride_y;
        virstrid_y = ver_virstride * stride_y;
        hw_regs->comm_gen.reg013.h26x_error_mode = 1;
        hw_regs->comm_gen.reg013.h26x_streamd_error_mode = 1;
        hw_regs->comm_gen.reg013.colmv_error_mode = 1;
        hw_regs->comm_gen.reg021.error_deb_en = 1;
        hw_regs->comm_gen.reg021.inter_error_prc_mode = 0;
        hw_regs->comm_gen.reg021.error_intra_mode = 1;

        hw_regs->comm_gen.reg017.slice_num = dxva_cxt->slice_count;
        hw_regs->comm_paras.reg64.rps_mode = 0;
        hw_regs->comm_paras.reg64.frame_orslice = 0;
        hw_regs->comm_paras.reg64.stream_mode = 0;
        ...
        if (MPP_FRAME_FMT_IS_FBC(mpp_frame_get_fmt(mframe))) {
            ...
            hw_regs->comm_gen.reg012.fbc_e = 1;
            hw_regs->comm_gen.reg018.y_hor_virstride = fbc_hdr_stride >> 4;
            hw_regs->comm_gen.reg019.uv_hor_virstride = fbc_hdr_stride >> 4;
            hw_regs->comm_gen.reg020_fbc_payload_off.payload_st_offset = fbd_offset >> 4;
        } else {
            hw_regs->comm_gen.reg012.fbc_e = 0;
            hw_regs->comm_gen.reg018.y_hor_virstride = stride_y >> 4;
            hw_regs->comm_gen.reg019.uv_hor_virstride = stride_uv >> 4;
            hw_regs->comm_gen.reg020_y_virstride.y_virstride = virstrid_y >> 4;
        }
    }
    mpp_buf_slot_get_prop(cfg->frame_slots, dxva_cxt->pp.CurrPic.Index7Bits,
                          SLOT_BUFFER, &framebuf);
    hw_regs->comm_addr.reg130_decout_base  = mpp_buffer_get_fd(framebuf); //just index need map
    /*if out_base is equal to zero it means this frame may error
    we return directly add by csy*/

    if (hw_regs->comm_addr.reg130_decout_base == 0) {
        return 0;
    }
    fd =  mpp_buffer_get_fd(framebuf);
    hw_regs->comm_addr.reg130_decout_base = fd;
    mv_buf = hal_bufs_get_buf(reg_ctx->cmv_bufs, dxva_cxt->pp.CurrPic.Index7Bits);
    hw_regs->comm_addr.reg131_colmv_cur_base = mpp_buffer_get_fd(mv_buf->buf[0]);

    hw_regs->comm_paras.reg65_cur_top_poc = dxva_cxt->pp.CurrPicOrderCntVal;
    ...
    /* cabac table */
    hw_regs->comm_addr.reg197_cabactbl_base    = reg_ctx->bufs_fd;
    /* pps */
    hw_regs->comm_addr.reg161_pps_head_base         = reg_ctx->bufs_fd;
    hw_regs->comm_addr.reg163_rps_base         = reg_ctx->bufs_fd;

    hw_regs->comm_addr.reg128_rlc_base        = mpp_buffer_get_fd(streambuf);
    hw_regs->comm_addr.reg129_rlcwrite_base   = mpp_buffer_get_fd(streambuf);
    stream_buf_size                             = mpp_buffer_get_size(streambuf);
    hw_regs->comm_gen.reg016_str_len              = ((dxva_cxt->bitstream_size + 15)
                                                     & (~15)) + 64;
    hw_regs->comm_gen.reg016_str_len = stream_buf_size > hw_regs->comm_gen.reg016_str_len ?
                                       hw_regs->comm_gen.reg016_str_len : stream_buf_size;

    aglin_offset =  hw_regs->comm_gen.reg016_str_len - dxva_cxt->bitstream_size;
    if (aglin_offset > 0) {
        memset((void *)(dxva_cxt->bitstream + dxva_cxt->bitstream_size), 0,
               aglin_offset);
    }
    hw_regs->comm_gen.reg010.dec_e                = 1;
    hw_regs->comm_gen.reg011.dec_timeout_e        = 1;
    hw_regs->comm_gen.reg012.wr_ddr_align_en      = (dxva_cxt->pp.tiles_enabled_flag != 0)
                                                    ? 0 : 1;
    hw_regs->comm_gen.reg012.colmv_compress_en    = COLMV_COMPRESS_EN;

    if (mpp_get_soc_type() == ROCKCHIP_SOC_RK3588) {
        hw_regs->comm_gen.reg026.swreg_block_gating_e = 0xfffef;
        hw_regs->comm_gen.reg024.cabac_err_en_lowbits = 0;
        hw_regs->comm_gen.reg025.cabac_err_en_highbits = 0;
    } else {
        hw_regs->comm_gen.reg024.cabac_err_en_lowbits = 0xffffdfff;
        hw_regs->comm_gen.reg025.cabac_err_en_highbits = 0x3ffbf9ff;
        hw_regs->comm_gen.reg026.swreg_block_gating_e = 0xfffff;
    }

    hw_regs->comm_gen.reg011.dec_clkgate_e    = 1;
    hw_regs->comm_gen.reg011.dec_e_strmd_clkgate_dis = 0;
    hw_regs->comm_gen.reg026.reg_cfg_gating_en = 1;
    hw_regs->comm_gen.reg032_timeout_threshold = 0x3ffff;

    valid_ref = hw_regs->comm_addr.reg130_decout_base;
    reg_ctx->error_index[syn->dec.reg_index] = dxva_cxt->pp.CurrPic.Index7Bits;
    hw_regs->comm_addr.reg132_error_ref_base = valid_ref;
    ...
    for (i = 0; i < (RK_S32)MPP_ARRAY_ELEMS(dxva_cxt->pp.RefPicList); i++) {
        if (dxva_cxt->pp.RefPicList[i].bPicEntry != 0xff &&
            ...
            hw_regs->comm_paras.reg67_82_ref_poc[i] = dxva_cxt->pp.PicOrderCntValList[i];
            mpp_buf_slot_get_prop(cfg->frame_slots,
                                  dxva_cxt->pp.RefPicList[i].Index7Bits,
                                  SLOT_BUFFER, &ref_buf);
            mpp_buf_slot_get_prop(cfg->frame_slots, dxva_cxt->pp.RefPicList[i].Index7Bits,
                                  SLOT_FRAME_PTR, &mframe);
            if (!ref_buf && mpp_get_soc_type() == ROCKCHIP_SOC_RK3588 &&
                ...
                if (!reg_ctx->missing_ref_buf) {
                    ...
                    mpp_buffer_get(cfg->buf_group, &reg_ctx->missing_ref_buf, reg_ctx->missing_ref_buf_size);
                    if (!reg_ctx->missing_ref_buf) {
            ...
            if (ref_buf) {
                hw_regs->comm_addr.reg164_179_ref_base[i] = mpp_buffer_get_fd(ref_buf);
                valid_ref = hw_regs->comm_addr.reg164_179_ref_base[i];
            ...
            if (reg_ctx->error_index[syn->dec.reg_index] == dxva_cxt->pp.CurrPic.Index7Bits)
                SET_POC_HIGNBIT_INFO(hw_regs->h2645_highpoc, i, poc_highbit, 3);
        }
        hw_regs->comm_addr.reg181_196_ref_colmv_base[i] = mpp_buffer_get_fd(mv_buf->buf[0]);
    }

    if ((reg_ctx->error_index[syn->dec.reg_index] == dxva_cxt->pp.CurrPic.Index7Bits) &&
        !dxva_cxt->pp.IntraPicFlag && !cfg->cfg->base.disable_error) {
    ...
    /* rps */
    mpp_dev_set_reg_offset(cfg->dev, 163, reg_ctx->rps_offset);

    hw_regs->comm_gen.reg013.timeout_mode = 1;
    hw_regs->comm_gen.reg013.cur_pic_is_idr = dxva_cxt->pp.IdrPicFlag;//p_hal->slice_long->idr_flag;

    hw_regs->comm_gen.reg011.buf_empty_en = 1;

    hal_h265d_rcb_info_update(hal, dxva_cxt, hw_regs, width, height);
    vdpu34x_setup_rcb(&hw_regs->comm_addr, cfg->dev, (reg_ctx->fast_mode != 0) ?
                      reg_ctx->rcb_buf[syn->dec.reg_index] : reg_ctx->rcb_buf[0],
                      (VdpuRcbInfo*)reg_ctx->rcb_info);
    vdpu34x_setup_statistic(&hw_regs->comm_gen, &hw_regs->statistic);
...
static MPP_RET hal_h265d_vdpu34x_start(void *hal, HalTaskInfo *task)
{
    ...
    if (task->dec.flags.parse_err ||
        (task->dec.flags.ref_err && !reg_ctx->cfg->cfg->base.disable_error)) {
        h265h_dbg(H265H_DBG_TASK_ERR, "%s found task error\n", __FUNCTION__);
        return MPP_OK;
    }
    ...
    if (reg_ctx->fast_mode) {
        ...
        hw_regs = ( Vdpu34xRegSet *)reg_ctx->g_buf[index].hw_regs;
    } else {
    ...
    do {
        ...
        MppDev dev = reg_ctx->cfg->dev;

        wr_cfg.reg = &hw_regs->comm_gen;
        wr_cfg.size = sizeof(hw_regs->comm_gen);
        ...
        wr_cfg.reg = &hw_regs->comm_addr;
        wr_cfg.size = sizeof(hw_regs->comm_addr);
        ...
        if (mpp_get_soc_type() == ROCKCHIP_SOC_RK3588) {
            wr_cfg.reg = &hw_regs->h2645_highpoc;
            wr_cfg.size = sizeof(hw_regs->h2645_highpoc);
...
ERR_PROC:
    if (task->dec.flags.parse_err ||
        ...
        if (!reg_ctx->fast_mode) {
            if (cfg->dec_cb)
                mpp_callback(cfg->dec_cb, &task->dec);
        } else {
            MppFrame mframe = NULL;
        ...
        if (reg_ctx->fast_mode && reg_ctx->fast_mode_err_found) {
            for (i = 0; i < (RK_S32)MPP_ARRAY_ELEMS(task->dec.refer); i++) {
                if (task->dec.refer[i] >= 0) {
                    ...
                    mpp_buf_slot_get_prop(cfg->frame_slots, task->dec.refer[i],
...
MPP_DEC_HAL_API_REGISTER(hal_h265d_vdpu34x)
